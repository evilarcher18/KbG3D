...
//Sareta marrazteko funtzioa
void draw_grid(){
	glPointSize(10);
	glLineWidth(5);
	glBegin(GL_LINES);  //Puntuaren marrazketaren hasiera
	for(double i=-25.0; i<25.0; i+=0.25){  //Marra horizontalen altuera eta haien arteko espazioa kontrolatzeko i aldagaiaren arabera
	  glColor3f(0.5,0.5,0.5); //Marra horizontalaren kolorea
	  glVertex2f(-25.0,i); //Marra marrazteko behar diren bi puntuen espezifikazioa
	  glVertex2f(25.0,i);  //Marra marrazteko behar diren bi puntuen espezifikazioa
	  glColor3f(0.5,0.5,0.5); //Marra bertikalaren kolorea
	  glVertex2f(i,-25.0); //Marra bertikala marrazteko puntuak lerro honetan eta hurrengoan
	  glVertex2f(i,25.0);
	}
	glEnd();
}
